...
\subsection{\textit{Design} de gamificação: Octalysis e Bartle}
...
são apresentados os \textit{core drives} que, se utilizados de maneira correta, podem aumentar o interesse e engajamento dos perfis identificados na seção anterior.
...
\begin{table}[h]
	...
	Predadores (\textit{Killers}) & & x & x & x & x & & & x   \\ \hline
	Realizadores (\textit{Achievers}) & & x & x & x & & x & & \\ \hline
	Exploradores (\textit{Explorers}) & & x & x & & & x & x & \\ \hline
	Socializadores (\textit{Socializers}) & & & x & x & x & & x & \\ \hline
	\end{tabular}%
	}
...
Como apresentado no capítulo anterior, existem quatro perfis de jogadores quando se leva em consideração o \textit{framework Octalysis},
...
da \textit{University of Waterloo}, que possui como propósito a identificação desses perfis.
...
contendo XXX perguntas referentes à disciplina Algoritmos de Programação de Computadores, algumas outra perguntas
...
os perfis predominantes dos jogadores levando em consideração o \textit{framework Octalysis}. Em seguida, o questionário foi publicado em um grupo na rede social \textit{facebook} que contém
